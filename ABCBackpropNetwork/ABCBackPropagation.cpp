 ...
 *       The code to execute this via terminal is "g++ -I tomlplusplus/include -std=c++20 ABCBackPropagation.cpp", and
      ...
      while (iterations < maxIterations && errorReached > errorThreshold)
      {
         ...
         for (caseNum = 0; caseNum < numCases; ++caseNum)
         {
            ...
            for (J = 0; J < numHiddenActivations; ++J)
            {
               ...
               for (I = 0; I < numOutputActivations; ++I)
               {
                  capitalOmega += lowerPsi[I] * weightsIJ[I][J];
                  weightsIJ[I][J] += lambda * h[J] * lowerPsi[I];
               } // for (I = 0; I < numOutputActivations; ++I)

               capitalPsi[J] = capitalOmega * activationFunctionDerivative(thetaJ[J]);
